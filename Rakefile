...
def changelog_user
  ...
  return_val = nil || JSON.load(File.read('metadata.json'))['author']
  raise _('unable to find the changelog_user in .sync.yml, or the author in metadata.json') if return_val.nil?
...
def changelog_project
  ...
  raise _('unable to find the changelog_project in .sync.yml or calculate it from the source in metadata.json') if return_val.nil?
...
def changelog_future_release
  return unless Rake.application.top_level_tasks.include? 'changelog'
  return_val = format('v%s', JSON.load(File.read('metadata.json'))['version'])
  raise _('unable to find the future_release (version) in metadata.json') if return_val.nil?
